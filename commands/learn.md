...
## Step 4 (`--fold`) — quarterly graduation

Rules that have held for a quarter (or that the operator names) graduate: fold the instruction into `vault/voice/style-dna.md` or `banned-patterns.md` (phrase bans go in the frontmatter list so `wr sweep` enforces them mechanically — the best possible outcome for a rule is becoming a regex). Mark the ledger entry `Status: superseded — folded into <file> <date>`. Never delete entries; the ledger is also the system's history of being wrong.

## Close

`wr check`. Report: items classified and routed · rules appended (quoted) · diffs mined · what pends on the operator.
