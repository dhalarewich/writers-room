...
3. `wr ship <id>`. The tool captures the diff between agent-final and what the operator actually shipped — the single highest-signal voice data this system collects.
4. If the diff exists, say one line: how many changed hunks, and that `/learn` will mine it. Do not mine it now — shipping should feel like shipping, not like a review.
5. Remind once: paste the piece to the actual channel yourself — this studio never posts anywhere.
