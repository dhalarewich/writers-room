...
1. **Resolve the studio first.** Run `wr board --json`. If it errors, you are not in a studio — offer `wr init` or `/setup`. Never operate on a guessed path. If the studio exists but writing is about to happen for the first time, `wr doctor --json` tells you whether the vault is still template — an unseeded studio routes to `/setup`, not to the Writer (drafting on template voice files produces generic text and erodes trust in the whole room).
